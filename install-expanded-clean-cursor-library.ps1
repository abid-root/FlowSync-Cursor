...
$zipName = "flowsync-cursor-library-expanded-clean.zip"
...
$backup = Join-Path (Split-Path $root -Parent) "flowsync-before-expanded-clean_$stamp"
$temp = Join-Path $env:TEMP "flowsync-expanded-clean-$stamp"
...
$fresh = Join-Path $temp "flowsync-cursor-library"
if (!(Test-Path $fresh)) { throw "Fresh project folder not found inside ZIP." }
...
Write-Host "DONE: Expanded clean cursor library installed."
Write-Host "Added: Simple Essentials, Animal Friends, Tiny Toys."
Write-Host "Now you have 12 categories and 60 effects."
Write-Host "Backup saved here:"
Write-Host $backup
Write-Host "Open with Live Server and press Ctrl + F5."
