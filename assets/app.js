...
function initTheme() {
  ...
  qsa("[data-theme-toggle]").forEach((btn) => {
    btn.onclick = (event) => {
      ...
      apply(clean(root.getAttribute("data-theme")) === "light" ? "dark" : "light");
    };
  });
...
function renderCategoryGrid(grid, activeId, sameFolder, expanded = false) {
  ...
  function card(cat, panel = false) {
    const href = sameFolder ? `${cat.id}.html` : `categories/${cat.id}.html`;
    return `
      <a class="category-card ${cat.id === activeId ? "active" : ""} ${panel ? "panel-card" : ""}" href="${href}">
        <strong><span class="cat-icon" aria-hidden="true">${cat.icon}</span><span class="cat-label">${cat.title}</span></strong>
      </a>`;
  }

  grid.innerHTML = `
    <div class="category-main-row">${visible.map((cat) => card(cat)).join("")}</div>
    <div class="category-more-panel" aria-hidden="${expanded ? "false" : "true"}">
      <div class="more-panel-title">All categories</div>
      <div class="more-panel-grid">${COLD_DATA.map((cat) => card(cat, true)).join("")}</div>
    </div>`;
...
function initSourcePage(effectKey) {
  ...
  if (preview && effect) attachEffectPreview(preview, effect);
...
(function setupHideBarsOnScroll() {
  ...
  window.addEventListener("scroll", () => {
    ...
    requestAnimationFrame(() => {
      ...
      document.body.classList.toggle("hide-top-bars", down);
      if (y < 40 || y < lastY) document.body.classList.remove("hide-top-bars");
      lastY = y;
      ticking = false;
    });
    ticking = true;
